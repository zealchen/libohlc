...
fn test_ohlc_make_data_a() {
    ...
    if let Ok(lines) = read_lines(&ohlc_path) {
        for line in lines {
            ...
            let ohlc: OHLCData =
                serde_json::from_str(ip.as_str()).expect("JSON was not well-formatted");
    ...
    for (i, reference) in references.into_iter().enumerate() {
        assert_eq!(
            predictions[i].open.parse::<f64>().unwrap(),
            reference.open.parse::<f64>().unwrap()
        );
        assert_eq!(
            predictions[i].high.parse::<f64>().unwrap(),
            reference.high.parse::<f64>().unwrap()
        );
        assert_eq!(
            predictions[i].low.parse::<f64>().unwrap(),
            reference.low.parse::<f64>().unwrap()
        );
        assert_eq!(
            predictions[i].close.parse::<f64>().unwrap(),
            reference.close.parse::<f64>().unwrap()
        );
    }
