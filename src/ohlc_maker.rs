use crate::tools::datas::{OHLCData, OHLCWindow, TickData};
...
impl OHLCMaker {
    ...
    pub fn make(
        ...
        let mut out = std::fs::File::create(ohlc_path).expect("create failed");
        for item in ohlc_datas {
            out.write_all((serde_json::to_string(&item).unwrap() + "\n").as_bytes())
                .expect("write fail.");
        }
    }

    // multi thread solution
    pub fn parallel_make(
        &self,
        tick_path: &str,    // input dataset path
        window_length: u64, // input window length in ms
        ohlc_path: &str,    // output ohlc path
        ...
        let ohlc_datas = self.make_ohlc_p(tick_datas, window_length);
    ...
    pub fn make_ohlc_p(
        &self,
        tick_datas_r: Arc<RwLock<Vec<TickData>>>,
        window_length: u64,
    ) -> Vec<OHLCData> {
        ...
        let ohlc_data_splits: Arc<Mutex<HashMap<u32, Vec<OHLCData>>>> =
        ...
        for i in 0..split_count {
            ...
            let thread_handle = thread::spawn(move || {
                ...
                let range_begin = splits.get(&i).unwrap().range_begin;
                let range_end = splits.get(&i).unwrap().range_end;
                let window_begin = splits.get(&i).unwrap().window_begin;

                // println!("range: {} {} {}", range_begin, range_end, window_begin);
                let tick_datas = c_shared_tick_datas.read().unwrap();
                let ohlc_split = make_batch_ohlc(
                    &tick_datas,
                    window_length,
                    range_begin,
                    range_end,
        ...
        for i in 0..split_count {
            ...
            // println!("add batch: {} {}", i, ohlc_split.len());
    ...
    fn split_tick_data(
        ...
        split_count: usize,
        window_length: u64,
    ) -> Arc<RwLock<HashMap<usize, TickDataRangeIndex>>> {
        // Split tick datas into split_count of parts.
        // Return range index and window index of all parts.

        let splits: Arc<RwLock<HashMap<usize, TickDataRangeIndex>>> =
            Arc::new(RwLock::new(HashMap::new()));
        let mut split_size = tick_datas.len() / split_count;
        if split_size == 0 {
            split_size = 1;
        }
        for split_index in 0..split_count {
            if split_index == 0 {
                splits.write().unwrap().insert(
                    split_index,
                    TickDataRangeIndex {
                        range_begin: 0,
                ...
                splits.write().unwrap().insert(
                    ...
                    TickDataRangeIndex {
                        ...
                        window_begin: {
                            let mut window_begin = split_index * split_size;
                            while window_begin > 0 {
                                if tick_datas[window_begin].T - tick_datas[window_begin - 1].T
                                    <= window_length
                                {
                                    window_begin -= 1;
                                } else {
...
fn update_window(
    ...
    if cur_tick.T - begin_tick.T > window_length {
        ...
        for (i, tick_data) in tick_datas
            .iter()
            .enumerate()
            .skip(window.begin_index + 1)
            .take(cur_index + 1)
        {
            if tick_data.s.eq(symbol) && cur_tick.T - tick_data.T < window_length {
                window.begin_index = i;
                window.open = tick_data.price.unwrap();
        ...
        for (_, tick_data) in tick_datas
            .iter()
            .enumerate()
            .skip(window.begin_index + 1)
            .take(cur_index + 1)
        {
            let price = tick_data.price.unwrap();
...
pub fn make_batch_ohlc(
    tick_datas: &[TickData], // all the tick datas
    ...
    for index in window_begin..range_end + 1 {
        ...
        if !symbel_windows.contains_key(&tick.s) {
            ...
            update_window(
                tick_datas,
                symbel_windows.get_mut(&tick.s).unwrap(),
                cur_price,
                index,
                window_length,
            );
